...
두 스킬로 구성된다: 위임 하네스 `gemini-delegate`와 설치 마무리용 `setup-gemini-delegate`.

**온라인 (권장)** — Agent Skills 공개 표준을 쓰는 `skills` CLI로 한 번에 설치한다. 두 스킬이
호스트의 스킬 디렉터리(`~/.claude/skills/` 또는 `~/.codex/skills/`)로 들어간다.

```bash
npx skills@latest add genius-kim-samsung/gemini-delegate
```

**오프라인·사내망 (npm 접근 불가)** — 저장소를 받아 설치기로 복사한다.

```bash
git clone <이 저장소>
cd gemini-delegate
python install.py     # ~/.claude/skills/ 로 두 스킬 복사
```

Codex를 오프라인으로 쓰면 `skills/` 아래 두 디렉터리를 `~/.codex/skills/`로 복사한다.

**설치 후 — 능동 위임 넛지 켜기 (권장):** 에이전트에서 `/setup-gemini-delegate` 를 실행한다.
이 스킬이 호스트를 감지해(Claude Code→`CLAUDE.md`, Codex→`AGENTS.md`) "대량 작업 전 위임을
먼저 검토하라"는 넛지를 전역 메모리 파일에 마커 블록으로 넣는다. **스킬 파일을 복사하는 것만으로는
능동 발동이 켜지지 않는다** — 이 한 줄이 긴 세션에서도 위임을 잊지 않게 한다(배경: `docs/adr/0004`).
`/delegate` 또는 "Gemini에게 시켜"로 명시 요청(반응 발동)은 이 넛지 없이도 동작한다.

요구사항: Python 3.8+(오프라인 설치기), **Gemini CLI**(사내 엔터프라이즈 워커, 설치·로그인).
...
호스트를 감지해 메모리 파일의 넛지 블록과 두 스킬 디렉터리를 함께 지운다. npx로 깔았다면
`npx skills remove gemini-delegate` 로 스킬 파일만 지울 수도 있으나, 그건 넛지 블록을 남긴다 —
...
└── setup-gemini-delegate/
    └── SKILL.md      # 능동 넛지를 호스트 메모리 파일(CLAUDE.md/AGENTS.md)에 넣고/빼는 스킬
```
...
이 스킬은 사내망 전용이지만, 유지보수는 gemini에 접근할 수 없는 사외망(집)에서 한다.
