...
description: >-
  gemini-delegate 설치를 마무리하는 일회성 스킬. 능동 위임 넛지("대량 작업 전 위임을
  먼저 검토하라")를 호스트 메모리 파일(Claude Code→CLAUDE.md, Codex→AGENTS.md)에
  마커 블록으로 넣거나 뺀다. 사용자가 "/setup-gemini-delegate", "위임 넛지 설치",
...
메모리 파일에 들어가는 한 줄 넛지다. 이 스킬이 그 넛지를 넣고/뺀다.

배경·경계는 `docs/adr/0004`, 용어(능동/반응 발동)는 `CONTEXT.md`.

## 모드

- **apply** (기본): 넛지 블록을 넣거나 최신으로 갱신한다.
- **remove**: 넛지 블록과 두 스킬 디렉터리를 제거한다 (사용자가 "제거/삭제/uninstall"이라 할 때).

## 1. 호스트 감지 — 어느 메모리 파일인가

능동 넛지는 **모든 프로젝트에 걸친** 리마인드이므로 감지된 호스트의 **전역** 메모리 파일을
대상으로 한다. 순서대로 판별하라:
...
`npx skills remove gemini-delegate`로 스킬 파일만 지울 수도 있으나, 그건 넛지 블록을 지우지
않는다 — 블록 제거는 이 스킬의 remove가 담당한다.

## 5. 경계 — 하지 말 것

- 블록 마커 밖의 사용자 내용을 수정·재배치하지 마라.
- 프로젝트별 메모리 파일이 아니라 **전역** 파일을 대상으로 하라(능동 넛지는 프로젝트 횡단).
