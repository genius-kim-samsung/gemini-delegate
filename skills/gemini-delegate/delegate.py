...
백엔드:
  auto (기본)   — PATH에서 gemini를 먼저 찾고, 없으면 agy를 쓴다
  gemini        — 사내 엔터프라이즈 Gemini CLI
  agy           — Antigravity CLI. 개인 계정용 Gemini CLI 후속 (사외망용)
...
def resolve_backend(backend):
    """(백엔드 이름, 실행 파일 경로)를 반환한다. auto는 gemini 우선으로 자동 감지."""
    if backend != "auto":
        exe = find_worker(backend)
        if not exe:
            sys.exit(f"오류: {backend} CLI를 PATH에서 찾을 수 없습니다.")
        return backend, exe
    for candidate in ("gemini", "agy"):
        exe = find_worker(candidate)
        if exe:
            return candidate, exe
    sys.exit("오류: 워커 CLI(gemini 또는 agy)를 PATH에서 찾을 수 없습니다.")
...
def main():
    ...
    parser.add_argument("--backend", choices=("auto", "gemini", "agy"),
                        default=os.environ.get("GEMINI_DELEGATE_BACKEND", "auto"),
                        help="워커 CLI. auto=gemini 우선 자동 감지 "
                             "(기본: auto, 환경변수 GEMINI_DELEGATE_BACKEND로 고정 가능)")
