...
- **자동 트리거는 setup 스킬 단독 소유**: CLAUDE.md/AGENTS.md 마커 블록을 넣고 빼는 로직은 `setup-gemini-delegate` 스킬에만 있다. install.py는 스킬 복사만 한다(자동 트리거 로직을 Python에 중복 두지 마라). 배포 경계는 `docs/adr/0004`.
